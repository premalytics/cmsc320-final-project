...
While this project did manage some interesting results, there
are still several areas of improvement. One issue is that by only
working with the average genre ratings for each user, we lost
information about variance for each genre per user. This means that
certain users may end up in the same cluster based on similar average ratings
but still rate books wildly differently because variation was not accounted for.

Another area of improvement was potentially using a more advanced clustering algorithm.
K-means clustering was not able to distinguish some of the smaller clusters in group 6,
possibly due to large amount of users in other locations occupying the algorithm.
As there were more data points densely packed in the area near groups 2 and 5, the centroids
for K-means ended up gathering near there and ignoring smaller groups elsewhere.
