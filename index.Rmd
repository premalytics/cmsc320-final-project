...
training_data <- user_features_df %>%
  mutate(cluster=kmeans_res$cluster) %>% sample_n(2000) #otherwise we have way too much data
...
rf_curve_df <- do.call(rbind, res)

#now we get AUC results:
rf_auc_df <- rf_curve_df %>% group_by(fold) %>%
