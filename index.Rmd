...
In general, data is split into two classes for, one for training the model and the other for testing the model.
Let's take a sample of our users and split them into training and testing data.
...
Now that we have split our data, let's use it to test to types of classification models, random forests and state vector machines.
...
For a given classification, every tree makes its own decision and all results are aggregated
...
Let us a train a random forest with 200 trees.
...
The importance of a predictor is determined using the **Mean Decrease in Accuracy**, which is the decrease
...
predictor were removed, it would result in the model making on average more misclassifications than the second.
...
Observing the graph, it seems that the average ratings for realistic fiction and horror are
able to best able classify users since they show the greatest mean decrease in accuracy on
removal.
...
![SVM Example: Increasing dimensions allows for an easier separation](images/svm_example.png)
...
Let's plot both the confusion matrix for our random forest and SVM models
on the test data.
...
Cursory observation of both matrices seems to indicate that while
both models are quite good at predicting clusters based on ratings,
