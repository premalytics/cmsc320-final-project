...
An issue with using K-means is that we do not have a good idea of what is a "good" K. We can
try to address this by doing several test runs with different K's and comparing them, specifically,
...
# Figure out a good k using SSE
sse <- sapply(1:10, function(k) {
  sum(kmeans(user_features_df, k)$withinss)
})

ggplot(mapping=aes(x=seq_along(sse), y=sse)) +
  geom_line() +
  geom_point() +
  labs(
    title="SSE vs K in K-Means Clustering",
    x="k",
    y="SSE"
  )
```

Looking at the graph, a K of about 8 seems reasonable to proceed with.

**TODO: do silhouette too**
**TODO: More research and talk about kmeans**
...
kmeans_res <- kmeans(user_features_df, 8)
kmeans_res$centers
